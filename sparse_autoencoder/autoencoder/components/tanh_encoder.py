...
class TanhEncoder(Module):
    r"""Linear encoder layer with the Tanh(ReLU()) activation function.

    Linear encoder layer (essentially `nn.Linear`, with a Tanh(ReLU()) activation function).
    Designed to be as the encoder in a sparse autoencoder (excluding any outer tied bias).
    To avoid the sparse autoencoder from learning to reduce L1 loss by reducing the scale of the
    feature activation, we add Gaussian noise to the output before the activation function so that
    small activations are heavily perturbed.
    ...
    \begin{align*}
        ...
        \text{Act} &= \text{Tanh}(\text{ReLU}) \\
        \epsilon \in \mathbb{R}^{b \times m} &= \text{Gaussian noise}(0, \sigma) \\
        f &= \text{Act}(\overline{\mathbf{x}} W_e^T + b_e + \epsilon) = \text{TanhEncoder output}
    \end{align*}
    $$
    """
