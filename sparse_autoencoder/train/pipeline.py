...
from sparse_autoencoder.metrics.validate.feature_suppression import FeatureSuppressionMetric
...
class Pipeline:
    ...
    def __init__(
        ...
        self.feature_suppression_metric = ClasswiseWrapperWithMean(
            FeatureSuppressionMetric(len(cache_names), keep_batch_dim=False),
            component_names=cache_names,
            prefix="validation/norm_suppression",
        )
        self.feature_suppression_metric.to(get_model_device(self.source_model))
    ...
    def validate_sae(self, validation_n_activations: PositiveInt) -> None:
        ...
        with torch.no_grad():
            ...
            self.feature_suppression_metric.update(
        ...
        if wandb.run is not None:
            ...
            log.update(self.feature_suppression_metric.compute())
            wandb.log(log)
